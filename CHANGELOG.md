...
- 新增 `tests/release-check.sh` 并串入 `tests/smoke.sh`，覆盖 clean / dirty 仓库、验证未完成、archive 未完成、handover 缺失、疑似 secret 只报 `path:line`、active change ready 等发布前软检查边界。
...
- `bin/zimaflow` 新增只读 `release-check` / `release-check --json`：发布前汇总 git 状态、active change 的验证 / archive / handover 状态、疑似 secret 位置和四个发布前人工确认问题，输出 `ready` / `need_verify` / `need_archive` / `need_handover` / `need_secret_review` / `need_manual_confirmation`；不 deploy、不打 tag、不创建 GitHub Release、不读取发布 token 或云厂商凭证，secret 只输出 `path:line`。
