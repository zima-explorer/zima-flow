...
description: >
  存量老项目冷启动 / 项目考古 Skill。用于接手已有代码库、让 AI 快速建立项目全局认知， 产出架构总览、接口清单、数据模型/ER
  笔记和隐性知识问答记录。 触发词：老项目冷启动、项目考古、接手老项目、梳理架构、生成接口清单、生成 ER 图、项目 onboarding、legacy
  project bootstrap。
sync: false
...
本 Skill 不替代 `installer`：

- `installer` 负责把项目接入 zimaflow。
- `legacy-project-onboarding` 负责理解存量系统。
- 本 Skill 只推荐用于老项目 / 存量代码 / 项目考古；新项目初始化不强制生成认知底座。

本 Skill 产出的整体称为 **code-intelligence baseline**。baseline 是轻量、工具无关的：可以使用代码图谱工具增强，但不能把 graphify、CRG、Codegraph 等工具作为硬依赖。

可选增强：如果当前环境可用 `codebase-memory-mcp` 且项目已索引，可调用 `code-graph-to-diagram` 生成带证据字段的图表 Brief / SVG，作为 baseline 文档的补充材料。此增强必须可跳过，不得阻断 onboarding。
...
如果项目尚未完成 zimaflow 初始化，先建议走 `installer`；用户明确跳过时继续，但在输出中标记 `⚠️ 未初始化路径`。
...
| 文件/目录 | 内容 |
|------|------|
| `Designs/diagrams/` | 由 `code-graph-to-diagram` / `technical-diagram` 生成的 scoped Mermaid/SVG 图、`brief.yaml`、`NOTES.md` |
...
- `codebase-memory-mcp` 当前可用
- 目标项目已索引
- 本次 onboarding 范围已经明确到全项目概览 / 指定模块 / 指定链路之一
- 图表可以通过 path、function、route、depth 等 scope 控制在可读范围内

调用 `code-graph-to-diagram`，优先生成 1-3 张补充图：

| 场景 | 查询 / 图表 | 写入位置 |
|---|---|---|
| 架构总览 | `get_architecture` → `architecture` | `Designs/Architecture-Overview.md` |
| 模块地图 | scoped `get_architecture` → `architecture` / `mindmap` | `Designs/Module-Map.md` |
| 关键链路 | `trace_path(mode=calls/data_flow/cross_service)` → `sequence` / `dataflow` | `Designs/Key-Flows.md` |
| 影响面 | `trace_path(direction=both)` → `architecture` / `mindmap` | `Designs/Key-Flows.md` 或需求相关 Decisions |

保存约定：

```text
Designs/diagrams/<slug>/
- brief.yaml
- diagram.mmd      # Markdown 文档优先，可选
- diagram.svg
- NOTES.md
```
...
- 在对应 baseline 文档中只引用图表路径和关键结论，不把长查询日志塞进正文。
- Markdown 文档优先保存 `diagram.mmd`；需要 `md-to-html` 稳定内联、演示或独立预览时同时保存 `diagram.svg`。
- `brief.yaml` 必须保留 `source` / `evidence` / `assumptions` / `warnings`。
- 静态调用图必须标注"静态代码图推断，不是 runtime trace"。
- 代码推断数据模型必须标注"非真实 DB schema"。
- 如果 MCP 查询失败、项目未索引、结果过大或图不可读，记录"代码图谱增强：跳过 / 失败原因"，继续后续 onboarding。
...
baseline:
  ...
  diagram_refs: []   # optional: list only 1-5 high-value diagram directories, not every diagram file
  code_graph_evidence:
    status: unavailable
    notes: ""
...
- `context-index.yaml` 默认记录包含图表的 baseline 文档路径（如 `architecture_overview` / `key_flows`），不要把所有图表文件摊平成清单。
- `baseline.diagram_refs` 只在图表本身是高价值入口时使用，最多记录 1-5 个图表目录（如 `Designs/diagrams/webpage-publish/`），不要记录每个 `diagram.mmd` / `diagram.svg` 文件；具体文件由该目录下的 `brief.yaml` / `NOTES.md` 说明。
- `baseline.code_graph_evidence` 只记录状态与短备注，不记录长查询日志。
- 常用命令只记录命令字符串，不记录长日志。
...
如果本次 onboarding 读取或应用了锚点表中的知识（规则见 `knowledge-anchor-guide.md`），按 `knowledge-usage-contract.md` 追加 ledger 事件。
