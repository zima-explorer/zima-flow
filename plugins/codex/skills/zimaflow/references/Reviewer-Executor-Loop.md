...
delta spec 递归发现 `specs/**/spec.md`，以相对目录的完整 ASCII/NFC `capability_id` 与 requirement/scenario 组成 canonical triple；symlink 文件或祖先、越界解析、非法/超长 segment 一律 fail closed。Requirement/Scenario identity 由唯一共享函数按 NFC、Unicode lowercase、再次 NFC、保留 Unicode Letter/Number、其余连续字符折叠为单个 `-` 的顺序生成；禁止 NFKC、转写或翻译。空 identity 以 `delta_spec_identity_invalid` 拒绝，归一化碰撞继续返回 `delta_spec_id_collision`；纯 ASCII identity 与 flat v1 canonical digest bytes 保持不变。flat-only 既有 manifest schema/normalization v1 继续按原两字段 projection 产生完全相同的 digest bytes；namespaced/mixed Change 使用 schema/normalization v2，并在 manifest、matrix、receipt、review-ready/accepted 与 whole/release aggregate 中显式冻结完整 triple 和版本。v1 不得携带 `capability_id`，也不得伪装 namespaced coverage；unsupported 或交叉版本分别返回 `verification_subject_version_unsupported`、`verification_subject_version_mismatch` 或 `verification_subject_schema_upgrade_required`。
...
`blocked` 不在 generic transition table 中回到 executing。唯一恢复入口是 `zimaflow reviewer-executor resume --change <change> --authorization repo://...`：独立的 user/reviewer approval evidence 记录批准事实，executor-owned authorization envelope 只绑定其 path/hash、当前 change/objective/round、blocked event 与 required decision。handler 固定写入 `actor: executor`，不得从 CLI/envelope 自报 user actor；时间必须满足 `blocked_at < approved_at <= envelope_created_at <= resume_event_at`。成功只追加 `objective_resumed` 并保持同一 objective/round；每个后续 review/acceptance/whole/release gate 都从当前 tracked Git truth 重算双 artifact，缺失、覆盖、未跟踪或 identity 漂移均 fail closed。resume 本身不先运行 matrix/manifest current-validity，所以旧 no-manifest namespaced blocker 可先获批恢复，再在 review-ready 前升级 matrix v2。
