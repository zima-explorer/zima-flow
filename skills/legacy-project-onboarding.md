...
如果本次 onboarding 读取或应用了 `$ZIMAFLOW_HOME/references/knowledge-anchor-map.md` 中的知识，且项目启用了知识使用账本，可按 `$ZIMAFLOW_HOME/references/knowledge-usage-guide.md` 追加 ledger 事件。未启用账本时，只在输出中列出 knowledge ID 和使用状态。
