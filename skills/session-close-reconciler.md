...
读取文档同步矩阵：
```bash
cat "$ZIMAFLOW_DIR/references/doc-sync-matrix.md"
```
...
1. 检查本轮是否读取过 `references/knowledge-anchor-map.md`、`references/lessons-common.md` 或项目 `lessons.md`。
2. 列出本轮涉及的 knowledge ID，按 `loaded` / `cited` / `applied` / `challenged` 分类。
3. 检查 `references/knowledge-usage-ledger.jsonl` 是否已有对应事件。
