...
当某个分节的结论是"本轮无适用项"时，**不要单独输出一个只有一句话的空节**。把当轮所有此类分节合并成一行，放在清单末尾：

```markdown
*本轮无适用项：Knowledge Usage、Guardrail 收口*
```

**边界：这只是显示合并，不减少检查项。** 所有分项仍然要按各自的 Step 逐项检查，只是在结果为"不适用"时折叠显示。禁止退化成"不检查就不显示"——没检查过的项不能计入这一行，只有确认过确实不适用的才能折叠。有实际内容的分节（哪怕只有一条 ✅）仍单独成节。**🧠 Learn 候选不参与折叠**：未命中时仍须单独写明"未发现高置信候选"（见 Step 3.5）。

**规则 B：验证失败 / 证据不完整必须上浮**

验证结果不能只出现在正文里而不进入顶部三级状态，否则只看顶部会得到"没有缺失"的错误印象。以下两种情况必须上浮，不得省略：

- 本轮测试 / 验证**存在失败** → 必须写入 ❌ 明确缺失。
- 验证**未跑**，或证据不完整（说不清跑了什么、结果如何）→ 必须写入 📝 建议补充。

**即使失败已归因为环境问题、隔离副作用或与本轮改动无关的既有失败，仍然必须上浮**，只在条目中标注归因，例如：`全量测试 1 项失败（环境差异导致，非本轮回归）`。不允许因为"不是本次引入的"就从三级状态中省略。

边界：这仍是 soft check。上浮只要求"必须显示"，不自动阻断流程、不自动改测试、不自动重跑；是否处理由用户决定。
...
*（结论为"本轮无适用项"的分节按规则 A 折叠到这一行；🧠 Learn 候选不参与折叠）*
